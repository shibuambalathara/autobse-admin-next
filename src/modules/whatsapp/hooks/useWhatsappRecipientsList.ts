...
export function useWhatsappRecipientsList() {
  ...
  const { data, loading, refetch } = useQuery<WhatsappRecipientsResult>(
    WHATSAPP_RECIPIENTS_QUERY,
    {
  ...
  const recipients = data?.whatsapp?.whatsapp ?? [];
  const total = data?.whatsapp?.messageCount ?? 0;
  const eventOptions =
