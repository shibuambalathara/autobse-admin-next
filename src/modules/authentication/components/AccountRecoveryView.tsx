...
export function AccountRecoveryView() {
  ...
  const onVerifyOtp = async (data: { otp: string }) => {
    ...
    try {
      ...
      router.replace(ROUTES.userResetPassword);
    } catch (error: unknown) {
      setFormError(extractGraphqlError(error).message);
    }
  };
        ...
        <CardContent className="space-y-6 p-6 sm:p-8">
          ...
          <div className="space-y-1">
            <h2 className="text-xl font-semibold text-brand-900 sm:text-2xl">
              {AUTH_COPY.recoveryTitle}
            </h2>
            <p className="text-sm text-brand-500">{AUTH_COPY.recoveryDescription}</p>
          </div>
          ...
          {step === "verify" ? (
            ...
            <form
              onSubmit={mobileForm.handleSubmit(onSendOtp)}
              className="space-y-5"
            >
              <FormField
                label="Mobile number"
                htmlFor="recovery-mobile"
                required
